...
| in-flight piece | **#6b–#6d** — the rest of piece 6 (frames/rate/anchors, one movement speed, collision boxes/hitboxes/scaling/sheet coordinates) on `arena/01a0cd44-undertale`; see the piece-6 sub-piece table in §3 |
| next ⬜ piece | **#7** the shopkeeper visual sweep, then **#8** the acceptance matrix + final merge |
| in-flight open PR | this branch's piece-6b–6d PR (opened by the session that wrote it) |
...
the order, and all four are done: 6a (size), 6b (frames, rate, anchors), 6c
(movement speed) and 6d (collision boxes/hitboxes, scaling, sheet coordinates).
Piece 7 is next.
