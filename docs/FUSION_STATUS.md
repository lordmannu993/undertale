...
| in-flight piece | **#6b frame selection, animation rate, render anchors** and **#6c one movement speed** (`arena/01a0cd44-undertale`) — see the piece-6 sub-piece table in §3 |
| next ⬜ piece | **#6d** collision boxes/hitboxes, scaling and sheet coordinates, then **#7** the shopkeeper visual sweep. Do not skip to piece 7 |
| in-flight open PR | this branch's 6b/6c PR (opened by the session that wrote it) |
...
| 6 | §12 | Asset compatibility layer — `sprite_get_width/height` return cropped size at ~95 sites; per-frame origins; movement speed (UT 3 px/frame, Yellow +2 autorun); hitboxes/collision; the 980 `unresolved[]` crop offsets (never guess) | 🚧 **6a ✅, 6b ✅, 6c ✅, 6d ⬜** | `port/assetcompat.lua`, `port/graphics.lua`, `port/runtime.lua`, `port/controller.lua`, `port/frisk.lua`, `port/yellow_studio.lua`, `tools/recover_sprite_offsets.py`, `tools/yellow/assets.py`, `port/recovered_sprite_metadata.json`, `port/sprite_offsets.json`, `tools/convert.py`, `tests/test_asset_sizes.py`, `tests/test_asset_frames.py`, `docs/PORTING.md` | 6a: `tests/test_asset_sizes.py` (13 tests). 6b: `tests/test_asset_frames.py` (6 tests). 6c: `tests/test_movement_speed.py` (3 tests) — see the sub-piece table below |
...
the order: 6a (size), 6b (frames, rate, anchors) and 6c (movement speed) are done;
6d (collision boxes/hitboxes, scaling, sheet coordinates) is pending and must be
finished before piece 7.
