...
| field | value |
| --- | --- |
| most recent merged piece | **#5a Shared core Player progression** — [PR #41](https://github.com/lordmannu993/undertale/pull/41), merged to `master` as `33ae6c9` on 2026-09-22; [CI + native gate 35708911107](https://github.com/lordmannu993/undertale/actions/runs/35708911107) green |
| in-flight piece | *(none — 5a merged; parent #5 remains in progress)* |
| next ⬜ piece | **#5c unified player controllers and shared abilities**; do not skip to #7 |
| in-flight open PR | *(none for implementation — start 5c from master)* |
...
| 5 | §1 §3 §9–§11 §13 §14 | **Unified Player state** (split into 5a–5d below): one Player record + shared item table, projected at world boundaries, serialised as `Player` + `World` blocks; crossing zeroes `flag[0..29]` (do not park state there) | 🚧 **5a+5b merged; 5c–5d pending** | `port/player.lua`, `port/inventory.lua`, `port/runtime.lua`, `port/travel.lua`, `port/storage.lua`, `port/merge.lua`, `port/frisk.lua`, `tools/item_catalog.py`, `tests/test_unified_player_state.py`, `tests/test_unified_inventory.py`, `docs/` | 5a: [PR #41](https://github.com/lordmannu993/undertale/pull/41), 23 targeted tests. 5b: shared catalog/inventory/equipment + adapters, 13 targeted tests. Full suites green with CI + native gate each. Parent piece is **not complete** |
