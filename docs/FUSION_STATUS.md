...
| field | value |
| --- | --- |
| most recent merged piece | **#6a sprite canvas + size compatibility** — [PR #48](https://github.com/lordmannu993/undertale/pull/48), merged to `master` as `fedae36` on 2026-09-23; [CI + native gate 35831922600](https://github.com/lordmannu993/undertale/actions/runs/35831922600) green |
| in-flight piece | **#6b frame selection, animation rate, render anchors** (`arena/01a0cd44-undertale`) — see the piece-6 sub-piece table in §3 |
| next ⬜ piece | **#6c** movement speed as one shared layer, then **#6d** collision boxes/hitboxes, scaling and sheet coordinates, then **#7** the shopkeeper visual sweep. Do not skip to piece 7 |
| in-flight open PR | this branch's 6b PR (opened by the session that wrote it) |
...
| 6 | §12 | Asset compatibility layer — `sprite_get_width/height` return cropped size at ~95 sites; per-frame origins; movement speed (UT 3 px/frame, Yellow +2 autorun); hitboxes/collision; the 980 `unresolved[]` crop offsets (never guess) | 🚧 **6a ✅, 6b ✅, 6c–6d ⬜** | `port/assetcompat.lua`, `port/graphics.lua`, `port/runtime.lua`, `port/controller.lua`, `port/frisk.lua`, `port/yellow_studio.lua`, `tools/recover_sprite_offsets.py`, `tools/yellow/assets.py`, `port/recovered_sprite_metadata.json`, `port/sprite_offsets.json`, `tools/convert.py`, `tests/test_asset_sizes.py`, `tests/test_asset_frames.py`, `docs/PORTING.md` | 6a: `tests/test_asset_sizes.py` (13 tests). 6b: `tests/test_asset_frames.py` (6 tests) — see the sub-piece table below |
...
the order: 6a (size) and 6b (frames, rate, anchors) are done; 6c (movement speed)
and 6d (collision boxes/hitboxes, scaling, sheet coordinates) are pending and must
be finished before piece 7.
...
| **6c** | Movement speed as one shared layer (spec §12 "movement speed"): Undertale's walk step (3 px) and Yellow's `plspd` + 2 sprint as one rule the shared controller owns, instead of Undertale's extra 3px lattice step. Collides like both worlds' own movement. | ⬜ |
| **6d** | Collision boxes and hitboxes, scaling and GMS2 sheet coordinates (spec §12): precise masks read in canvas coordinates (a cropped export's pixels start at its recovered offset); Studio 2's per-frame precise masks (`collisionKind` 4 is *Precise Per Frame* in the GMS2 sprite schema, not a rotated rectangle); `draw_sprite_stretched`/particle sprites/tiled draws scale from the canvas; `sprite_get_uvs` answers from the frame. | ⬜ |
