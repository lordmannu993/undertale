...
| 5 | §1 §3 §9–§11 §13 §14 | **Unified Player state** (split into 5a–5d below): one Player record + shared item table, projected at world boundaries, serialised as `Player` + `World` blocks; crossing zeroes `flag[0..29]` (do not park state there) | 🚧 **5a+5b merged; 5c implemented, not yet merged; 5d pending** | `port/player.lua`, `port/inventory.lua`, `port/controller.lua`, `port/runtime.lua`, `port/travel.lua`, `port/storage.lua`, `port/merge.lua`, `port/frisk.lua`, `tools/item_catalog.py`, `tests/test_unified_player_state.py`, `tests/test_unified_inventory.py`, `tests/test_unified_controller.py`, `docs/` | 5a: [PR #41](https://github.com/lordmannu993/undertale/pull/41), 23 targeted tests. 5b: [PR #43](https://github.com/lordmannu993/undertale/pull/43), 13 targeted tests. 5c: 5 targeted tests. Parent piece is **not complete** (5d, then piece 8) |
