...
| field | value |
| --- | --- |
| most recent merged piece | **#3** River Person's boat and water (§6) — [PR #38](https://github.com/lordmannu993/undertale/pull/38), merged to `master` |
| in-flight piece | **#4 No duplicated characters/sprite layers (§8)** — local suite green (485 passed, 1 skipped) on the session branch; PR and merge follow |
| next ⬜ piece | **#5 Unified Player state (§1 §3 §9–§11 §13 §14)** — starts after #4 lands |
| in-flight open PR | *(the piece-#4 PR, opened from the session branch)* |
