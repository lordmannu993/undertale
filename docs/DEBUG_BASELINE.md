...
| --- | --- | --- |
| 1. Yellow font wrong | **reproduced** | `obj_dialogue` (and 10 more `obj_dialogue*` Creates) sets `dialogue_font = 9`, and `scr_initialize_battle` sets `global.font_type_text = 1` — Yellow's own compiled font numbers (pinned Asset_Order: 1 = `fnt_main_battle`, 9 = `fnt_main`). The merged build reads them through Undertale's band: **9 = `fnt_papyrus`, 1 = `fnt_main`**, so Yellow dialogue draws with Undertale fonts. Static references (`draw_set_font(E, 1000005)` etc.) resolve correctly; only the *raw-number* spellings are broken. Objects got a re-band (`Runtime:resolveObjectIndex`); fonts/sprites/sounds have none. **D1a's job:** close that gap where the value is consumed (`draw_set_font`/`state.font` resolution by caller world), reusing the existing `assetName`/band machinery — and check every other raw-number font path the probe's `draw_set_font` tap reveals. |
