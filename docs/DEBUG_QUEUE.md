...
| field | value |
| --- | --- |
| most recent merged piece | **D0** — architecture map + reproduction harness (brief §9): `docs/DEBUG_BASELINE.md`, `tools/debug_probe.py`, `tests/test_debug_probe.py`; PR from `arena/01a0d3db-undertale` merged 2026-09-24. No behaviour change; the harness reproduces the wrong-font and vertical-text symptoms headlessly, quantifies the Yellow-side tick cost, shows the packaged crossings keep exactly one Player, and records the boat-destination matrix incl. the hold-X ambiguity and the ride-skipping pager latch |
| in-flight piece | **none** |
| next ⬜ piece | **D1a** — Yellow uses its intended font (brief §1) |
