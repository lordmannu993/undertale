...
| field | value |
| --- | --- |
| most recent merged piece | **D1a** — Yellow uses its intended font (brief §1): `Runtime:resolveFontIndex` (sibling of `resolveObjectIndex`) re-bands a Yellow caller's raw font number through the merged ID band at `draw_set_font`; PR from `arena/01a0d7e8-undertale`. Root cause: Yellow's `dialogue_font = 9` / `global.font_type_text = 1` are Yellow's own compiled font numbers (`fnt_main` / `fnt_main_battle`) that the merged build was reading as Undertale's `fnt_papyrus` / `fnt_main`. Undertale callers and already-banded ids are unchanged |
| in-flight piece | **none** |
| next ⬜ piece | **D1b** — Yellow text renders horizontally (brief §1 orientation, §6) |
...
| --- | --- | --- | --- | --- | --- |
| **D0** | §9 | **Architecture map + reproduction.** Trace GAME START → room init → player init → Undertale play → River Person → boat → transition → Yellow room → player update/draw → dialogue draw → return transition → Undertale room; write down where state changes, how the two worlds are separated, and which code owns fonts, text, rooms, entities and transitions. Stand up one repeatable headless reproduction harness that prints the four reported symptoms (Yellow font, vertical text, Yellow lag, duplicate player at the crossing, boat destination) so every later piece starts from evidence. No behaviour change. | ✅ | new `docs/DEBUG_BASELINE.md`, new `tools/debug_probe.py`; read `main.lua`, `port/runtime.lua`, `port/graphics.lua`, `port/travel.lua`, `port/player.lua`, `port/merge.lua`, `port/yellow_*.lua`, `tools/convert.py`, `tools/yellow/*.py` | PR from `arena/01a0d3db-undertale` (merged 2026-09-24) / `tests/test_debug_probe.py` (4 tests; fail without the harness). Harness output quoted in `docs/DEBUG_BASELINE.md` §3: wrong font reproduced (`dialogue_font = 9` reads Undertale's `fnt_papyrus`, not Yellow's `fnt_main`), vertical text reproduced (`draw_text_ext(..., width=-1)` wraps every word: 7 words → 7 lines, 55×126 px box), Yellow tick cost quantified (snowdin ~2.9× a quiet UT room, 806 draws/frame; hotland at parity), duplicate Player **not** reproduced by 14 scripted crossings (max 1 Player), boat matrix 6/6 lands as documented + the hold-X world flip and the ride-skipping pager latch recorded. Behaviour unchanged: full suite green before and after |
| **D1a** | §1 (font identity) | **Yellow uses its intended font.** Trace font loading → record → ID → `draw_set_font` → renderer for both worlds; find why a Yellow caller gets the wrong face. Fix the shared resolution (caller's world / band / name map — reuse `Runtime:assetName`, `manifest.double_named`, the merged `fonts` records); never substitute a downloaded or generic font. Undertale's fonts verified unchanged. | ✅ | `port/runtime.lua` (`Runtime:resolveFontIndex`), `port/graphics.lua` (`draw_set_font`); inspected `port/merge.lua`, `port/yellow_builtins.lua`, `tools/convert.py`, `tools/yellow/assets.py` (static refs already rewritten; no converter change) | PR from `arena/01a0d7e8-undertale` / `tests/test_yellow_fonts.py` (7 tests; 5 fail without the re-band). Root cause: Yellow's decompiler leaves raw compiled font numbers (`dialogue_font = 9`, `global.font_type_text = 1`) which collided with Undertale's band; objects already had `resolveObjectIndex`, fonts did not. `obj_dialogue` now draws with Yellow's `fnt_main` (merged 1000009), not `fnt_papyrus`. Undertale `draw_set_font(9/1/2)` still selects papyrus/main/maintext; named `fnt_main` still splits by world. Vertical text is a different cause (D1b) |
