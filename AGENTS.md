...
| Published release | `love-v1.2.2-fusion-experimental` (prerelease, 4 assets incl. both conversion reports, checksum in notes), source `1538133`, published by the pinned workflow after **413 tests** and the fused native gate. Adds Snowdin Inn temporary maxhp +10 at every LV and carries merged PR #27's River Person Yellow destination fix, absent from v1.2.1. Prior releases stay available; v1.2.1 is renamed with a "Superseded —" prefix. Published release assets are immutable: do not re-publish over them |
