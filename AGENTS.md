...
recorded in its §1 (currently **D1b** — Yellow text renders horizontally, brief §1/§6).
...
the baseline before starting any D1–D5 piece; it records the reproduced wrong-font
and vertical-text evidence, the performance numbers, and the boat/player matrix.
**D1a is merged** (this branch): Yellow callers' raw font numbers re-band at
`draw_set_font` (`Runtime:resolveFontIndex`); Yellow dialogue uses `fnt_main`,
Undertale's faces are unchanged.
...
- Piece order: **D0** architecture map + a repeatable reproduction of all of the
  above (§9, merged) → **D1a** Yellow font identity (merged) → **D1b** vertical text + graphics-state
...
- [x] Publish `love-v1.2.5-fusion-experimental` through the draft-only
  workflow on `arena/01a0d296-undertale`. The first run
  ([35978059651](https://github.com/lordmannu993/undertale/actions/runs/35978059651))
  failed in pytest: the new headless assertion loaded the suite's real
  catalog. The re-run
  ([35979772170](https://github.com/lordmannu993/undertale/actions/runs/35979772170))
  passed **573 tests**, merged packaging, the native Linux LÖVE gate and
  the four-asset size/digest check. Source commit:
  `c16fb9b7932baedbef32a403ba2fb3b116a64226`. Archive:
  `undertale-yellow-fusion-v1.2.5-experimental.love`, 408,697,434 bytes.
  v1.2.4 is retitled with the "Superseded — " prefix. Published assets
  stay immutable; v1.2.4 was not rewritten.

## v1.2.4 release task (2026-09-24)

- [x] Carry the whole certified unified fusion — PRs #33-#53, pieces 1-8 of
  `docs/FUSION_STATUS.md` ("the queue has no pending piece") — in the next
  downloadable archive. v1.2.3 (source `6d59f5d`) predates all of it. This is
  the release build the queue pointed at after piece 8 landed.
- [x] Bump runtime, README download links, release notes and pinned publisher
  to v1.2.4 on `arena/01a0d251-undertale`; delete the README's "Merged after
  this archive was built" note, since this build contains that entire queue.
- [x] Publish `love-v1.2.4-fusion-experimental` through the draft-only
  workflow: [run 35971586512](https://github.com/lordmannu993/undertale/actions/runs/35971586512)
  passed all 571 tests, merged packaging, native Linux LÖVE and four-asset
  size/digest verification. Source commit:
  `3c3da8a574be30f1e4e85bf015abfc61158b41e5`. The README/source-commit
  follow-up is this branch's second metadata commit (no code changes).
- [x] Retitle the superseded v1.2.3 release with the "Superseded — " prefix.
...
| Open PR | **none** once this branch's D1a PR merges — **D0** (architecture map + reproduction harness) is merged from `arena/01a0d3db-undertale`; **D1a** (Yellow uses its intended font) is this branch. The next PR is **D1b** (Yellow text renders horizontally) |
| Live debug queue | The owner's [`DEBUG_SPEC.md`](docs/DEBUG_SPEC.md) (12-section binding brief) is worked as a **living queue** in [`docs/DEBUG_QUEUE.md`](docs/DEBUG_QUEUE.md) — the single GitHub-readable source of truth for what is done, what is pending (in order), the rules, and the "definition of a finished piece." **"Proceed ❤️" continues it.** Pieces are **D0** (architecture map + reproduction of the reported bugs — **merged 2026-09-24**: [`docs/DEBUG_BASELINE.md`](docs/DEBUG_BASELINE.md) + `tools/debug_probe.py`; reproduces the wrong font and the vertical text headlessly, quantifies Yellow tick cost, keeps the fusion invariants), **D1a** (Yellow font identity — **merged** this branch: `Runtime:resolveFontIndex` at `draw_set_font`; `tests/test_yellow_fonts.py`), **D1b** (vertical text), **D2** (Yellow-side lag), **D3** (exactly one Player across the crossing), **D4** (boat destination), **D5** (graphics-state + resource sweep), **D6** (both-side regression pass, cleanup, the ten-point report). One piece per PR, merged only once complete |
...
```bash
.venv/bin/python -m pytest -q                                   # headless suite (571 passed + 1 skipped at piece 8, PORT_REQUIRE_YELLOW=1)
