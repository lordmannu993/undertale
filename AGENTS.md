...
| Open PR | **none** — PR #24 merged into `master` as `f0849dc`. It carried the owner's rounds 1-2: `port/frisk.lua` asserts all 24 run poses stay Clover's and none enters the walk remap (the startup report counts the remap instead of `#remap`, which was always 0); PAUSE gains **AUTO RUN: ON/OFF** beside COLLISION - Yellow's own `option_autorun`, persisted in `touch-settings-v1.txt`, mirrored to the `Controls.sav` key Yellow's `scr_savecontrols` writes, re-applied on boot and after every crossing; and both travel services are open from the first frame - `Travel:openRiverService()` lifts `obj_dogboat_thing`'s own `global.plot < 122` guard for that one Create event (restored after, warn `travel-river-service`), `Travel:openWhaleService()` sets Yellow's `global.player_can_travel` and seeds ten stops through Yellow's own `scr_fasttravel_add` (warn `travel-ugps`), Yellow stops resolve through the merged ID band (`56` -> `1000056`, not Undertale's room 56), `Travel:landWhales()` reads the whale's unreachable `fly_speed == 0` landing as the landing it was written to be (warn `travel-ugps-landing`), and `port/runtime.lua` gives every viewport a `view_camera` so a mail-station bell no longer stops the frame on "Camera 0 does not exist". Tests: the boat at all three docks at plot 10, the whole bell -> Mail/Travel -> menu -> Yellow-stop flight, and every offered stop pinned against the pinned Yellow source (294 pass; CI `34868075393` green). Documented in `docs/YELLOW.md` and `docs/RELEASE_NOTES.md`. Known gap reported, not hidden: the "Snowdin - Forest" stop - and the Snowdin boat crossing, which lands in the same room - needs GameMaker's particle system, absent from this runtime, so `rm_snowdin_11_yellow` stays a named compatibility stop. Still open from round 1: "it bugs a lot" (no reproducible symptom) |
