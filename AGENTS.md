...
- [ ] Publish `love-v1.2.3-fusion-experimental` through the draft-only
  ...
  in the README once the workflow flips the draft.
- [ ] Retitle the superseded v1.2.2 release with the "Superseded — " prefix.
...
| Published release | `love-v1.2.3-fusion-experimental` (prerelease, 4 assets incl. both conversion reports, checksum in notes) is being published by the pinned workflow after the suite, merged packaging and the fused native gate; run link, source commit and test count are recorded in the v1.2.3 release task above once it flips. Carries PR #30's four phone fixes (dialogue-box depth −400, GMS2 touch built-ins, pager label layout, cached drawing) that post-date the v1.2.2 archive; v1.2.2 (`love-v1.2.2-fusion-experimental`, source `1538133`, **413 tests**) stays available and is retitled "Superseded —" when v1.2.3 lands. Published release assets are immutable: do not re-publish over them |
...
| Source newer than the archive | none while v1.2.3 publishes: v1.2.3's source is `arena/01a0b9dd-undertale` (master plus the v1.2.3 metadata commit), which carries [PR #30](https://github.com/lordmannu993/undertale/pull/30)'s four fixes that the v1.2.2 archive lacked. Once the draft flips and this branch lands on `master`, master and the newest archive agree again |
