...
| Published release | `love-v1.2.1-fusion-experimental` (prerelease, 4 assets incl. both conversion reports, checksum in notes) is the merged Undertale ⊕ Undertale Yellow build, published by the pinned workflow with the fused native gate — every stop travels: both travel services open from the first frame, and the particle system lands the Snowdin forest stop. `love-v1.2.0-fusion-experimental` and `love-v0.1.5`…`v0.1.10-experimental` stay up, renamed with a "Superseded —" prefix. Every published release is immutable: do not re-publish over it |
