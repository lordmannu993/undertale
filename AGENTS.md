...
- [ ] Publish `love-v1.2.4-fusion-experimental` through the draft-only
  workflow: run [TBD](https://github.com/lordmannu993/undertale/actions)
  carries the suite, merged packaging, native Linux LÖVE gate and four-asset
  size/digest verification; source commit and test count are recorded here and
  in the README once the workflow flips the draft. The README/source-commit
  follow-up is this branch's second metadata commit (no code changes).
- [ ] Retitle the superseded v1.2.3 release with the "Superseded — " prefix.
...
| Published release | `love-v1.2.4-fusion-experimental` (prerelease, 4 assets incl. both conversion reports, checksum in notes) is being published by the pinned workflow after the suite, merged packaging and the fused native gate; run link, source commit and test count are recorded in the v1.2.4 release task above once it flips. Carries the whole certified unified fusion (PRs #33-#53, pieces 1-8) that post-dates the v1.2.3 archive; v1.2.3 (`love-v1.2.3-fusion-experimental`, source `6d59f5d`, **421 tests**) stays available and is retitled "Superseded —" when v1.2.4 lands. Published release assets are immutable: do not re-publish over them |
...
| Source newer than the archive | none while v1.2.4 publishes: v1.2.4's source is `arena/01a0d251-undertale` (master `e2d6fb3` plus the v1.2.4 metadata commit), which carries the entire certified unified-fusion queue (PRs #33-#53) that the v1.2.3 archive lacked. Once the draft flips and this branch lands on `master`, master and the newest archive agree again |
