        ...
        if registry_file.exists():
            ...
            for category, entries in registry.get("recovered", {}).items():
                for name, evidence in entries.items():
                    if name in self.ids.get(category, {}) and self.ids[category][name] != evidence["id"]:
                        conflicts.append(dict(category=category, name=name, local=self.ids[category][name], upstream=evidence["id"]))
            self.report["registry_conflicts"] = conflicts
            self.report["registry_import"] = "voided: pinned upstream registry is audit/conflict-only; no IDs imported"
